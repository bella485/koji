...
class RepoWatcher(object):
        ...
        while True:
            ...
            for f in watch_fields:
                ...
                if val1 != val2:
                    if f == 'task_state':
                        # convert if we can
                        val1 = koji.TASK_STATES[val1] if val1 else val1
                        val2 = koji.TASK_STATES[val2] if val2 else val2
    ...
    def check_tag(self):
        ...
        err = None
        if check is None:
            err = "Tag %(name)s disappeared, perhaps renamed?" % self.taginfo
        elif check.get('revoke_event'):
            err = "Tag %(name)s has been deleted" % self.taginfo
        if err:
            raise koji.GenericError(err)

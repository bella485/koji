...
def load_plugin(plugin_type, plugin_name):
    ...
    PLUGIN_FILENAME = os.path.join(
        os.path.dirname(__file__),
        "../../plugins",
        plugin_type,
        "%s.py" % plugin_name)
    sys.path = [os.path.dirname(PLUGIN_FILENAME),
                os.path.join(os.path.dirname(__file__), "../..", plugin_type)] + \
               sys.path
    if sys.version_info[0] >= 3:
        import importlib.machinery
        loader = importlib.machinery.SourceFileLoader(mod_name, PLUGIN_FILENAME)
        ...
        import imp
        plugin = imp.load_source(mod_name, PLUGIN_FILENAME)
    return plugin

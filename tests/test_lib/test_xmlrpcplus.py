...
class TestDump(unittest.TestCase):
    ...
    def test_call(self):
        ...
        for value in self.standard_data:
            ...
            _enc = xmlrpc.client.dumps(value, methodname=method, allow_none=1)
            self.assertEqual(enc, _enc)
            params, method = xmlrpc.client.loads(enc)
    ...
    def test_response(self):
        for value in self.standard_data:
            ...
            _enc = xmlrpc.client.dumps(value, methodresponse=1, allow_none=1)
            self.assertEqual(enc, _enc)
            params, method = xmlrpc.client.loads(enc)
            self.assertEqual(params, value)
    ...
    def test_just_data(self):
        # xmlrpc.client supports this case, so I guess we should too
        # neither method call nor response
        for value in self.standard_data:
            value = (value, "foo", "bar")
            enc = xmlrpcplus.dumps(value)
            _enc = xmlrpc.client.dumps(value, allow_none=1)
            self.assertEqual(enc, _enc)
            params, method = xmlrpc.client.loads(enc)
            self.assertEqual(params, value)
            self.assertEqual(method, None)
    ...
    def test_fault(self):
        ...
        f2 = xmlrpc.client.Fault(code, msg)
        value = f1
        enc = xmlrpcplus.dumps(value, methodresponse=1)
        _enc = xmlrpc.client.dumps(value, methodresponse=1, allow_none=1)
        self.assertEqual(enc, _enc)
    ...
    def test_encoding(self):
        ...
        for value in data:
            ...
            _enc = xmlrpc.client.dumps(value, methodresponse=1, allow_none=1, encoding='us-ascii')
            self.assertEqual(enc, _enc)
            params, method = xmlrpc.client.loads(enc)
            self.assertEqual(params, value)
            self.assertEqual(method, None)
